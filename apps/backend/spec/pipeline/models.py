...
from init import init_turret_dir
...
def get_specs_dir(project_dir: Path, dev_mode: bool = False) -> Path:
    ...
    IMPORTANT: Only .turret/ is considered an "installed" turret.
    The turret/ folder (if it exists) is SOURCE CODE being developed,
    not an installation. This allows Turret to be used to develop itself.

    This function also ensures .turret is added to .gitignore on first use.
    ...
    Returns:
        Path to the specs directory within .turret/
    """
    # Initialize .turret directory and ensure it's in .gitignore
    init_turret_dir(project_dir)

    # Return the specs directory path
    return project_dir / ".turret" / "specs"
