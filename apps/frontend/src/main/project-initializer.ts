 ...
 */
const GITIGNORE_ENTRIES = ['.turret/'];
...
function ensureGitignoreEntries(projectPath: string, entries: string[]): void {
  ...
  if (existsSync(gitignorePath)) {
    ...
    writeFileSync(gitignorePath, '# Turret data directory\n' + entriesToAdd.join('\n') + '\n');
  }
...
/**
 * Data directories created in .turret for each project
 */
...
/**
 * Check if project is initialized (has .turret directory)
 */
export function isInitialized(projectPath: string): boolean {
  const dotAutoBuildPath = path.join(projectPath, '.turret');
  return existsSync(dotAutoBuildPath);
}

/**
 * Initialize turret data directory in a project.
 *
 * Creates .turret/ with data directories (specs, ideation, insights, roadmap).
...
export function initializeProject(projectPath: string): InitializationResult {
  ...
  if (!gitStatus.isGitRepo || !gitStatus.hasCommits) {
    ...
    return {
      success: false,
      error: gitStatus.error || 'Git repository required. Turret uses git worktrees for isolated builds.'
    };
  }
  ...
  if (existsSync(dotAutoBuildPath)) {
    debug('Already initialized - .turret exists');
    return {
      success: false,
      error: 'Project already has turret initialized (.turret exists)'
    };
  }

  try {
    debug('Creating .turret data directory', { dotAutoBuildPath });

    // Create the .turret directory
    ...
    // Update .gitignore to exclude .turret/
...
/**
 * Ensure all data directories exist in .turret.
...
/**
 * Get the turret folder path for a project.
 *
 * IMPORTANT: Only .turret/ is considered a valid "installed" turret.
 * The turret/ folder (if it exists) is the SOURCE CODE being developed,
 * not an installation. This allows Turret to be used to develop itself.
 */
export function getAutoBuildPath(projectPath: string): string | null {
  ...
  if (existsSync(dotAutoBuildPath)) {
    debug('Returning .turret (installed version)');
    return '.turret';
  }

  debug('No .turret folder found - project not initialized');
  return null;
}
